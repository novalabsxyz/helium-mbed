...
static bool wait_for_byte(Serial &serial, size_t timeout) {
...
namespace helium {
    ...
    // Atom
    //
    Atom::Atom(PinName tx, PinName rx, PinName power_pin)
        : serial(tx, rx, 115200), sequence(0) {

        if (power_pin != NC) {
            power = new DigitalOut(power_pin);
        }
